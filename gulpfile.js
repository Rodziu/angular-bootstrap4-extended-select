...
!function() {
	...
	function _bundle(production) {
		return gulp.src('dummy', {allowEmpty: true})
			.pipe(webpackStream({
				entry: {
					'angularjs-bootstrap4-extended-select': './.build/angularjs-bootstrap4-extended-select.js'
				},
				...
				output: {
					devtoolNamespace: 'extendedSelect',
					filename: (pathData) => {
	...
	exports.default = gulp.series('ts', 'scss', gulp.parallel('bundle', 'bundle:prod'));
}();
